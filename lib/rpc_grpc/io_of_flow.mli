(* Taken from ocaml-tron's lib/rpc_grpc/io_of_flow.ml, which is chain-agnostic:
   it turns a flow into the four functions gluten wants and knows nothing about
   Tron or Cosmos. Copied rather than shared because the two repositories have
   no common package below the chain layer, and a web3-codec-gluten holding one
   87-line functor would be a package for its own sake. If a third consumer
   turns up, that changes. *)
